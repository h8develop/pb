<template>
  <div class="sparkle-button">
    ...
    <span aria-hidden="true" class="particle-pen">
      <!-- Particle SVGs will be dynamically created here -->
    </span>
  </div>
</template>
...
const createParticles = () => {
  ...
  particles.value = Array.from({ length: 30 }, createParticle);
  ...
  particles.value.forEach((particle) => {
    ...
    particlePen.appendChild(svg);
  });
};

const updateParticleStyles = () => {
  particles.value.forEach((particle, index) => {
    const svg = document.querySelectorAll(".particle")[index];
    svg.style.setProperty("--x", `${particle.x}%`);
    svg.style.setProperty("--y", `${particle.y}%`);
    svg.style.setProperty("--duration", `${particle.duration}s`);
    svg.style.setProperty("--delay", `${particle.delay}s`);
    svg.style.setProperty("--alpha", particle.alpha.toString());
    svg.style.setProperty("--origin-x", `${particle.originX}%`);
    svg.style.setProperty("--origin-y", `${particle.originY}%`);
    svg.style.setProperty("--size", particle.size.toString());
  });
};
...
onMounted(() => {
  ...
  button.addEventListener("mouseenter", () => updateActive(true));
  button.addEventListener("mouseleave", () => updateActive(false));
  button.addEventListener("focus", () => updateActive(true));
  button.addEventListener("blur", () => updateActive(false));

  createParticles();
  updateParticleStyles();

  setInterval(updateParticleStyles, 1000);
});

const playState = computed(() => (active.value ? "running" : "paused"));
</script>
